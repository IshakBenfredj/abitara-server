  ...
  try {
    ...
    for (const item of items) {
      ...
      if (!product) {
        return res
          .status(400)
          .json({ message: `المنتج غير موجود: ${item.product}` });
      }

      if (!product.isActive) {
        return res
          .status(400)
          .json({ message: `المنتج غير متاح: ${product.title}` });
      }
      ...
      try {
        ...
        for (let chunk of chunks) {
          try {
            ...
            console.log("Push tickets from Expo:", ticketChunk);
...
exports.updateOrderStatus = async (req, res) => {
  try {
    ...
    res
      .status(400)
      .json({ message: "خطأ في تحديث حالة الطلب: " + error.message });
  }
};
...
exports.getOrderStats = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({
        message: "حدث خطأ في الحصول على إحصاءات الطلبات: " + error.message,
      });
  }
};
...
exports.getAdvancedStats = async (req, res) => {
  try {
    ...
    const totalDeliveryCost =
      profitStats.length > 0 ? profitStats[0].totalDeliveryCost : 0;
    ...
    const result = {
      ...
      totalOrders: salesStats.length > 0 ? salesStats[0].totalOrders : 0,
      averageOrderValue:
        salesStats.length > 0 ? salesStats[0].averageOrderValue : 0,
      netProfit,
...
exports.getOrdersByIds = async (req, res) => {
  try {
    ...
    if (!Array.isArray(ids) || ids.length === 0) {
      return res
        .status(400)
        .json({ message: "يجب إرسال مصفوفة تحتوي على معرّفات الطلبات" });
    }
